...
def read_sensors():
    ...
    for key, value in sensors.items():
        if value["is_connected"] is True:
		    ...
		    if datetime.today().month == 3 or datetime.today().month == 4 or datetime.today().month == 5:
                      	if sensor_reading > 14 or sensor_reading < 5:
                        	push = '1'
                           	message = 'Alert! Current temperature recoded is ' + str(sensor_reading)
                            	url = "http://localhost/fcm/push_notification.php?push="+push+"&message="+message
                            	response = urllib.urlopen(url)
                            	data = response.read()
                            	print data
                    if datetime.today().month == 6 or datetime.today().month == 7 or datetime.today().month == 8:
                        if sensor_reading > 27 or sensor_reading < 15:
                            	push = '1'
                            	message = 'Alert! Current temperature recoded is ' + str(sensor_reading)
                            	url = "http://localhost/fcm/push_notification.php?push="+push+"&message="+message
                            	response = urllib.urlopen(url)
		    ...
		    if datetime.today().month == 9 or datetime.today().month == 10 or datetime.today().month == 11:
			print datetime.today().month
                        if sensor_reading > 25 or sensor_reading < 13:
                            	push = '1'
			    	print sensor_reading
			    	print ref_temp
                            	messge = 'Alert! Current temperature recoded is ' + str(sensor_reading)
                            	url = "http://localhost/fcm/push_notification.php?push="+push+"&message="+message
                            	response = urllib.urlopen(url)
                            	data = response.read()
                            	print data
		    if datetime.today().month == 12 or datetime.today().month == 1 or datetime.today().month == 2:
			print datetime.today().month
			print sensor_reading

                        if sensor_reading > 12 or sensor_reading < 3:
                            	push = '1'
                            	title = 'Seaking'
                            	message = 'Alert! Current temperature recoded is ' + str(ref_temp)
                            	url = "http://localhost/fcm/push_notification.php?push="+push+"&msg="+ message
                            	response = urllib.urlopen(url)
                            	data = response.read()
                            	print data
    ...
    # Get the readings from any other Atlas Scientific sensors
            ...
            if value["sensor_type"] == "atlas_scientific":
                ...
                all_curr_readings.append([value["name"], sensor_reading])
		print sensor_reading
		if sensor_reading < 7 or sensor_reading > 8:
			push = '1'
			message = 'Alert! Current temperature recoded is ' + str(sensor_reading)
			url = "http://localhost/fcm/push_notification_ph.php?push="+push+"&message="+message
			response = urllib.urlopen(url)
			data = response.read()
			print data
...
sensors = OrderedDict([("temp_1", {  # DS18B20 Temperature Sensor
                            ...
                            "is_ref": False,
                            "ds18b20_file":
                            "/sys/bus/w1/devices/28-020f91774613/w1_slave",
                            "accuracy": 1}),
